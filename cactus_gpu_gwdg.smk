...
sys.path.append(
    os.path.abspath(os.path.join(os.path.realpath(workflow.basedir), "lib/"))
)
import cactuslib
...
# time -p apptainer exec --nv --cleanenv ../bin/cactus_v2.9.3-gpu.sif cactus-prepare evolverMammals.txt --outDir evolverMammals-out/ --outHal evolverMammals-out/evolverMammals.hal --gpu
...
apptainer_sif = config["cactus_path"]
conda_env = config["conda_env"]
TMPDIR = config["tmp_dir"]

# shell.executable("/bin/bash")
# shell.prefix("conda activate {conda_env} && ")

# A directory with lots of space to use for temporary files generated by the cactus-align command

CACTUS_PATH = "apptainer exec --nv --cleanenv " + apptainer_sif
CACTUS_PATH_TMP = (
    "apptainer exec --nv --cleanenv --bind " + TMPDIR + ":/tmp --bind /scratch:/scratch " + apptainer_sif
)
...
rule all:
    input:
        expand(
            ...
            final_tip=[tips[name]["output"] for name in tips],
        ),
        # The masked input files from rule mask
        expand(
            os.path.join(OUTPUT_DIR, "{internal_node}.fa"),
            internal_node=[node for node in internals],
        ),
...
rule mask:
    input:
        lambda wildcards: [
            tips[name]["input"]
            for name in tips
            if tips[name]["output"] == wildcards.final_tip
        ][0],
    output:
        os.path.join(OUTPUT_DIR, "{final_tip}"),
    conda:
    ...
    params:
        ...
        genome_name=lambda wildcards: [
            name for name in tips if tips[name]["output"] == wildcards.final_tip
        ][0],
        job_dir=lambda wildcards: os.path.join(
            TMPDIR,
            [name for name in tips if tips[name]["output"] == wildcards.final_tip][0]
            + "-mask",
        ),
        gpu_opt=GPU_OPT,
    resources:
        partition=config["mask_partition"],
        gpus=config["mask_gpu"],
        cpus=config["mask_cpu"],
        mem=config["mask_mem"],
        runtime=config["mask_runtime"],
    shell:
        """
        if [ -d {params.job_dir} ]; then
            {params.path} cactus-preprocess {params.job_dir} {params.input_file} {params.cactus_file} --inputNames {params.genome_name} --logInfo --retryCount 0 --maxCores {resources.cpus} {params.gpu_opt} --restart
        else
            {params.path} cactus-preprocess {params.job_dir} {params.input_file} {params.cactus_file} --inputNames {params.genome_name} --logInfo --retryCount 0 --maxCores {resources.cpus} {params.gpu_opt}
        fi
        """
...
rule blast:
    input:
        lambda wildcards: [
            ...
            for input_file in internals[wildcards.internal_node]["input-seqs"]
        ],
    output:
    ...
    params:
        ...
        job_dir=lambda wildcards: os.path.join(
            "/tmp", wildcards.internal_node + "-blast"
        ),
        gpu_opt=GPU_OPT,
    resources:
        partition=config["blast_partition"],
        gpus=config["blast_gpu"],
        cpus=config["blast_cpu"],
        mem=config["blast_mem"],
        runtime=config["blast_runtime"],
    ...
    shell:
        """    
        if [ -d {params.job_dir} ]; then
            {params.path} cactus-blast {params.job_dir} {params.cactus_file} {output} --root {params.node} --logInfo --retryCount 0 --lastzCores {resources.cpus} {params.gpu_opt} --restart
        else
            {params.path} cactus-blast {params.job_dir} {params.cactus_file} {output} --root {params.node} --logInfo --retryCount 0 --lastzCores {resources.cpus} {params.gpu_opt}
        fi
        """
...
rule align:
    ...
    params:
        ...
        job_dir=lambda wildcards: os.path.join(
            "/tmp", wildcards.internal_node + "-align"
        ),
        work_dir=TMPDIR,
        gpu_opt=GPU_OPT,
    resources:
        ...
        runtime=config["align_runtime"],
    shell:
        """
        if [ -d {params.job_dir} ]; then
            {params.path} cactus-align {params.job_dir} {params.cactus_file} {input.cigar_file} {output} --root {params.node} --logInfo --retryCount 0 --workDir {params.work_dir} --maxCores {resources.cpus} --defaultDisk 450G --gpu --restart
        else
            {params.path} cactus-align {params.job_dir} {params.cactus_file} {input.cigar_file} {output} --root {params.node} --logInfo --retryCount 0 --workDir {params.work_dir} --maxCores {resources.cpus} --defaultDisk 450G --gpu
        fi
        """
...
rule convert:
    ...
    resources:
        ...
        runtime=config["convert_runtime"],
    shell:
        """
...
rule copy_hal:
    input:
        all_hals=expand(
            os.path.join(OUTPUT_DIR, "{internal_node}.fa"), internal_node=internals
        ),
...
rule append:
    ...
    resources:
        ...
        runtime=config["append_runtime"],
    shell:
        """
        python append_hal.py \
            {input} \
            {OUTPUT_DIR} \
            {root_name} \
            "{internals}" \
...
#             #cmd = ["apptainer", "exec", "--nv", "--cleanenv", "--bind", TMPDIR + ":/tmp", config["cactus_path"], "halAppendSubtree", os.path.join(OUTPUT_DIR, root_name + ".hal"), os.path.join(OUTPUT_DIR, node + ".hal"), node, node, "--merge", "--hdf5InMemory"];
#             cmd = ["apptainer", "exec", "--nv", "--cleanenv", "--bind", TMPDIR + ":/tmp", config["cactus_path"], "halAppendSubtree", OUTPUT_HAL, os.path.join(OUTPUT_DIR, node + ".hal"), node, node, "--merge", "--hdf5InMemory"];
...
# Note that calling apptainer with --nv will print text to stderr even though there is no error
## End node loop
